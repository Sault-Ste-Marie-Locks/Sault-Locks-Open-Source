...
$log = Join-Path ([Environment]::GetFolderPath("Desktop")) "lock-release-import-log.txt"
...
try {
  ...
  try {
    ...
    Write-Host "Used /api/import/json endpoint."
    ...
    Write-Host "Import endpoint unavailable or failed: $($_.Exception.Message)"
