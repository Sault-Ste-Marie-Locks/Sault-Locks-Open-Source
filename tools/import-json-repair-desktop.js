...
function findJsonPath() {
  ...
  if (cliJson) return path.resolve(cliJson);
  const here = __dirname;
  const candidates = [
    path.join(here, '..', 'imports', 'sault-locks-today-2026-07-16.json'),
    path.join(here, '..', 'sault-locks-today-2026-07-16.json'),
    path.join(process.cwd(), 'sault-locks-today-2026-07-16.json')
  ];
  const found = candidates.find(p => fs.existsSync(p));
  if (!found) throw new Error('Import JSON was not found. Pass --json C:\\path\\to\\file.json');
  return found;
}
...
function main() {
  ...
  console.log('Lock Release GitHub/Desktop JSON import repair');
