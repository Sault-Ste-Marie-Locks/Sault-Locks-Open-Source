...
function recKey(r){
  const explicit=String(r?.dashboardId||r?.recordId||r?.record_id||'').trim();
  if(/^(mobile:|old:|id:|key:)/i.test(explicit)) return explicit;
  if(explicit) return 'id:'+explicit;
  if(r?.mobileId) return /^(mobile:|old:|id:|key:)/i.test(String(r.mobileId)) ? String(r.mobileId) : 'mobile:'+r.mobileId;
  if(r?.oldId) return /^(mobile:|old:|id:|key:)/i.test(String(r.oldId)) ? String(r.oldId) : 'old:'+r.oldId;
  if(r?.id) return /^(mobile:|old:|id:|key:)/i.test(String(r.id)) ? String(r.id) : 'id:'+r.id;
...
function updateTrafficFromMobile(existing, incoming, phone){
  const rec=normalizeMobileEntry(incoming||{}, phone||{});
  const existingRecordId=String(existing?.dashboardId||existing?.recordId||existing?.record_id||recKey(existing||{})).trim();
  const merged={...(existing||{}), ...rec};
  if(existingRecordId) merged.dashboardId=existingRecordId;
  if(existing?.id) merged.id=existing.id;
  if(existing?.oldId) merged.oldId=existing.oldId;
  if(existing?.mobileId) merged.mobileId=existing.mobileId;
  else if(rec.mobileId && !/^(mobile:|old:|id:|key:)/i.test(String(rec.mobileId))) merged.mobileId=rec.mobileId;
  else delete merged.mobileId;
